---------------------------- MODULE Migration ----------------------------
(* Track M: migration completion as a liveness theorem, and the starvation
   counterexample. Lean TLA+ port of the repaired protocol in
   prototypes/p4-agent-loop/protocol/migration.qnt (ghost read machinery
   dropped: it does not affect completion).

   BOUNDED = FALSE : app writes are unrestricted. Expected: TLC finds a
                     lasso where a writer keeps dirtying a key and every
                     backfill attempt aborts — completion is NOT guaranteed
                     (online-backfill starvation, machine-checked).
   BOUNDED = TRUE  : app writes draw from a finite budget (stand-in for
                     "interference eventually quiesces"). Expected: TLC
                     proves <>(phase = DONE) under weak fairness of the
                     migration actions alone (no fairness on app writes). *)
